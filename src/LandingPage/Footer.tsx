...
const Footer = () => {
 const location=useLocation();
    return location.pathname!="/signup"?<div className="pt-20 pb-5 flex gap-5 justify-around background-mine-shaft-950 font-[poppins']">
      <div className="w-1/4 flex flex-col gap-4">
        ...
        <div className="text-sm text-mine-shaft-300">
          Job portal with user profiles, skill updates,certifactions, work experience and admin job posting.
        </div>
      ...
      </div>
      {
        footerLinks.map((item,index)=><div key={index}>
          ...
          {
            item.link.map((link,index)=><div key={index} className="text-mine-shaft-300 text-sm hover:text-bright-sun-400 cursor-pointer mb-1 hover:translate-x-2 transition duration-300 ease-in-out">{link}</div>)
          }
          </div>)
      }
    </div>:<> </>
